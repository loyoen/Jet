...
void CServer::doTireState(LPDATA_STATE pState, LPPACKAGE onePackage, SOCKET mySocket)
{
...
void CServer::bindClient(SOCKET deviceSocket, SOCKET jetSocket)
{
	...
	if(pDeviceClient->getState()==STATE_READY)
	{
		...
		pOnePackage->pckHeader.dwLength = sizeof(DATA_STATE)+sizeof(tagPackageHeader);
		DATA_STATE myDataState;
		myDataState.iTireState =  STATE_READY;
		char* DataPack = (char*)malloc(sizeof(DATA_STATE));
		memcpy(DataPack,&myDataState,sizeof(myDataState));
		pOnePackage->pckBody.lpBuffer = DataPack;
...
void CServer::RecvPackage(LPPACKAGE pOnePackage,IOCompeletionData *pIOCompeletionData)
{
	...
	switch(pOnePackage->pckHeader.yType)
	{
		...
		case TAG_STATUE_DATA:
					memcpy(&myDataState,pOnePackage->pckBody.lpBuffer,sizeof(DATA_STATE));
					this->doTireState(&myDataState,pOnePackage,pIOCompeletionData->mySocket);
